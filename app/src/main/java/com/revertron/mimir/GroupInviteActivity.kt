...
import uniffi.mimir.decryptSharedKey as mimirDecryptSharedKey
...
class GroupInviteActivity : BaseActivity() {
    ...
    private var mediatorPubkey: ByteArray? = null
        ...
        override fun onReceive(context: Context?, intent: Intent?) {
            when (intent?.action) {
                "ACTION_INVITE_ACCEPTED" -> {
                    val acceptedChatId = intent.getLongExtra("chat_id", 0)
                    if (acceptedChatId == chatId) {
                        runOnUiThread {
                            dismissProgressDialog()
                            val chatIntent = Intent(this@GroupInviteActivity, GroupChatActivity::class.java).apply {
                                ...
                                putExtra(GroupChatActivity.EXTRA_IS_OWNER, false)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mediatorPubkey = intent.getByteArrayExtra("mediator_pubkey")
        ...
        val filter = IntentFilter().apply {
            addAction("ACTION_INVITE_ACCEPTED")
    ...
    private fun acceptInvite() {
        ...
        Thread {
            try {
                ...
                val sharedKey = mimirDecryptSharedKey(encryptedData, privkey)

                Log.i(TAG, "Decrypted shared key for chat $chatId")

                val mediatorPubkey = this.mediatorPubkey ?: MediatorManager.getDefaultMediatorPubkey()
                ...
                runOnUiThread {
                    showProgressDialog(R.string.joining_group)
                }
            } catch (e: Exception) {
