...
fun parseAndSaveGroupMessage(
    ...
    try {
        ...
        if (author.contentEquals(chatInfo.mediatorPubkey)) {
            ...
            // Handle chat deletion - mark chat as read-only
            if (sysMsg is SystemMessage.ChatDeleted) {
                Log.i(TAG, "Chat $chatId was deleted, marking as read-only")
        ...
        // Decrypt message using shared key (Rust ChaCha20-Poly1305)
        val decryptedData = try {
            mimirDecryptMessage(encryptedData, chatInfo.sharedKey)
        } catch (e: Exception) {
