...
const val MSG_TYPE_FILE_REQUEST = 4000
const val MSG_TYPE_FILE_RESPONSE = 4001
...
fun writeMessage(dos: DataOutputStream, message: Message, filePath: String, stream: Int = 0, type: Int = MSG_TYPE_MESSAGE_TEXT): Boolean {
    ...
    if (message.data.isNotEmpty()) {
        when (message.type) {
            1, 3 -> {
                ...
                attachedFile = file
                data = message.data  // only metadata; file bytes streamed below
                // payloadSize = metaJsonSize(4) + metaJson + fileBytes
                json.put("payloadSize", message.data.size + 4 + file.length())
            }
            else -> {
                data = message.data
                json.put("payloadSize", data.size)
            }
        }
    } else {
        data = message.data
    ...
    if (attachedFile != null) {
        dos.writeInt(jsonSize)        // metaJsonSize
        dos.write(data)               // metadata bytes
        attachedFile.inputStream().use { fis ->
            fis.copyTo(dos, bufferSize = 65536)   // stream file without loading into RAM
        }
