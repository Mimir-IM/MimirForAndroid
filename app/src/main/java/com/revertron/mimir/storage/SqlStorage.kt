...
class SqlStorage(val context: Context): SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    fun getMyPubKey(): ByteArray {
        if (myPublicKey == null) {
            var info = getAccountInfo(1, 0L)
            if (info == null) {
                generateNewAccount()
                info = getAccountInfo(1, 0L)
            }
            myPublicKey = (info!!.keyPair.public as Ed25519PublicKeyParameters).encoded
        }
    ...
    fun getGroupChat(chatId: Long): GroupChatInfo? {
        val cursor = readableDatabase.query(
            ...
            arrayOf("chat_id", "name", "description", "avatar", "mediator_pubkey", "owner_pubkey", "shared_key",
                "created_at", "updated_at", "last_message_time", "last_msg_id", "unread_count", "muted", "read_only"),
            "chat_id = ?",
    ...
    fun setGroupChatReadOnly(chatId: Long, readOnly: Boolean): Boolean {
        return try {
            val values = ContentValues().apply {
            ...
            if (rowsUpdated > 0) {
                Log.d(TAG, "Group chat $chatId read_only status updated to $readOnly")
                true
            } else {
                Log.w(TAG, "Failed to update read_only status for group chat $chatId")
                false
            }
        } catch (e: Exception) {
            Log.e(TAG, "Error updating read_only status for group chat $chatId", e)
            false
        }
    }
    ...
    fun addGroupMessage(chatId: Long, serverMsgId: Long?, guid: Long, author: ByteArray, timestamp: Long, type: Int, system: Boolean, data: ByteArray, replyTo: Long = 0L, fromSync: Boolean = false): Long {
        ...
        val senderId = if (author.contentEquals(chatInfo.mediatorPubkey) || author.contentEquals(oldMediatorPubkey)) {
            ...
            val id = if (cursor.moveToNext()) {
                ...
                // Author not in members table yet — create a stub entry so the
                // message is not silently lost (can happen during initial sync
                // if messages arrive before the member list is populated).
                Log.w(TAG, "Author not found in members table, creating stub for ${author.toHexString().take(16)}")
                updateGroupMemberInfo(chatId, author, null, null, null)
                // Re-query to get the newly created member's id
                val retryCursor = readableDatabase.query(
                    membersTable, arrayOf("id"),
                    "pubkey = ?", arrayOf(author.toHexString()), null, null, null, "1"
                )
                val retryId = if (retryCursor.moveToNext()) retryCursor.getLong(0) else {
                    Log.e(TAG, "Failed to create stub member for author")
                    retryCursor.close()
                    cursor.close()
                    return -1
                }
     ...
     */
    fun updateGroupMembersPermissionsAndOnline(chatId: Long, members: List<uniffi.mimir.GroupMember>) {
        val membersTable = "members_$chatId"
        ...
        try {
            for (member in members) {
                ...
                val values = ContentValues().apply {
                    put("permissions", member.permissions.toInt())
